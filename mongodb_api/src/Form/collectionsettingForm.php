...
if (isset($_SESSION['mongodb_token']) && $_SESSION['mongodb_token'] != ""){
	if (!empty($mongodb_collection)) {
		  ...
		  if (isset($webform)) {
			...
			$webform_elements_key = array_keys_multiple($webform_elements);
	...
	$form['document']['select_all'] = [
		...
		foreach($json_result as $resultkey => $resultValue):
			...
			if (($resultkey != "_id")) {		
				...
				if (is_array($resultValue) && isAsso($resultValue)){
					$form['document'][$i] = [
						'#type' => 'details',
						'#title' => $resultkey ,
						'#prefix' => '<div class="clearboth">',
					...
					if(isset($webform_elements[$resultkey])){
						$sub_webform_elements_key = array_keys_multiple($webform_elements[$resultkey]);
						$sub_webform_elements = $webform_elements[$resultkey];
					}else{
						$sub_webform_elements_key = $sub_webform_elements = array();
					}
					
					$form['document'][$i]['document'] = addsubleveldoc($resultkey, $resultValue, $resultkey, $sub_webform_elements, $sub_webform_elements_key);
				...
				// checked for multiple inputs
				$query = \Drupal::entityQuery('collection_field_relation')
					->condition('status', 1)
					->condition('field_mongodb_connection_ref', $_SESSION['mongodb_nid'], '=')
					->condition('field_collection_name', trim($mongodb_collection), '=')
					->condition('field_sub_category_key', $resultkey, '=');
				$sub_coll_ids = $query->execute();
				
				if(isset($rel_collection) && !empty($sub_coll_ids)){
				...
				if(!empty($coll_ids)){
					...
					$form['document'][$i]['relative_entity'] = array(
						...
						'#class' => 'value-field',
						'#attributes' => array('style' => 'float: left; max-width: 200px; margin: 10px;','disabled' => TRUE),
						'#theme_wrappers' => array(),
					);
				}
				...
				$form['document'][$i]['dropdown_sort'] = array(
					'#type' => 'select',	
					'#options' => array(
						'none' => 'None',
						'asc' => 'Ascending',
					...
					'#default_value' => $dropdown_sort,
					'#prefix' => '<div class="dropdown_sort">',
					'#suffix' => '</div>'							
				);
		...
		endforeach;	
//$form['document'][$i] = addsubleveldoc('dsafd');
...
public function validateForm(array &$form, FormStateInterface $form_state) {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
	...
	foreach($document_values as $document_value):
		if ((isset($document_value['webform_select']) && $document_value['webform_select'] == 1) || isset($document_value['document'])):
			if (isset($document_value['key'])) {
				...
				if(array_find_deep($document_value['document'],1)){
					...
					add_subleveldoc_submit($document_value['document']['key'],$document_value['document'],$webform_elements);
				}
				//array_push($webform_elements,add_subleveldoc_submit($document_value['document']['key'],$document_value['document']));
			}			
		endif;
    endforeach;	
...
function addsubleveldoc($resultKey, $resultValue, $nlevelKey, $webform_elements, $webform_elements_key)
{
	...
	foreach($resultValue as $key => $value):		
		if (is_array($value) && isAsso($value)) {
			...
			if(isset($webform_elements[$key])){
				$sub_webform_elements_key = array_keys_multiple($webform_elements[$key]);
				$sub_webform_elements = $webform_elements[$key];
			}else{
				$sub_webform_elements_key = $sub_webform_elements = array();
			}
			
			$form[$j]['document'] = addsubleveldoc($key, $value, $nlevelKey.'###'.$key, $sub_webform_elements, $sub_webform_elements_key);
		} else {
			...
			$query = \Drupal::entityQuery('collection_field_relation')
				...
				->condition('field_mongodb_connection_ref', $_SESSION['mongodb_nid'], '=')
				->condition('field_collection_name', trim($mongodb_collection), '=')
				->condition('field_sub_category_key', trim($nlevelKey.'###'.$key), '=');
			$sub_coll_ids = $query->execute();
...
function add_subleveldoc_submit($doc_key, $document_values, &$webform_elements){
	//$webform_elements = [];
	...
	//kint (strcmp($document_key,'key') == 0);
		...
		if (strcmp($document_key,'key') != 0):		
			if (!isset($document_value['document'])) {			
				...
				$webform_elements[$document_value['document']['key']][] = add_subleveldoc_submit($document_value['document']['key'],$document_value['document']);*/
				...
				if(array_find_deep($document_value['document'],1)){
					...
					add_subleveldoc_submit($document_value['document']['key'],$document_value['document'],$webform_elements[$doc_key]);
				}	
			}	
		/*else:
...
function array_keys_multiple(array $array)
{
    $keys = array();
    foreach ($array as $key => $value) {
        ...
        if (is_array($array[$key])) {
            $keys = array_merge($keys, array_keys_multiple($array[$key]));
        }
  }
    return $keys;
}
